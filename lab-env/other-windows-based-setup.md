...
We may have to also edit this file 
...
[corp] I tried following, assume on vpn , do nslookup in git bash to get private DNS and insert this DNS in resolv.conf get to curl working on WSL via vpn DNS resolution => not working

I tried to install (corp): pip3 install tuya-connector-python, but issue as python 3.6
# https://github.com/csuhan/ReDet/issues/14
So shows that WSL uses old python by default
-->
